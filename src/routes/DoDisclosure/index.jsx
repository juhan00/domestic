...
import DomesticList, {
  DisclosureLoader,
} from "@components/Disclosure/DisclosureList/DomesticList";
...
const DoDisclosure = () => {
  ...
  const listRendering = () => {
    ...
    for (let i = 0; i < limit; i++) {
      result.push(
        <div
          className={`box_ani turn${i + 1} ${isBoxLoader && "ani_on"}`}
          style={{ transition: `all ease-in-out ${0.5 + i * 0.1}s` }}>
          <DisclosureLoader />
        </div>,
      );
    }
  ...
  return (
    <div className="disclosure">
      ...
      />
      {!disclosureList ? (
        <div style={{ marginTop: "60px" }}>{listRendering()}</div>
      ) : (
        <>
          <DomesticList
            data={disclosureList}
