...
import { DisclosureListWrapper, DisclosureListLoaderWrapper } from "./style";
import HashLoader from "react-spinners/HashLoader";

export const DisclosureLoader = () => {
  return (
    <DisclosureListLoaderWrapper>
      <div className="hash_loader_wrapper">
        <HashLoader color={"#48a185"} size={30} />
      </div>
    </DisclosureListLoaderWrapper>
  );
};
              ...
              <tr key={index} onClick={(e) => handleButtonClick(li.rcept_no)}>
                ...
                <td>
                  <a
                    className="link"
                    target="_blank"
                    href={`https://dart.fss.or.kr/dsaf001/main.do?rcpNo=${li.rcept_no}`}>
                    {li.report_nm}
                  </a>
                </td>
                <td>{li.flr_nm}</td>
                ...
                <td>
                  {li.rm === "유" && (
                    <span style={{ backgroundColor: "#FDE3E1" }}>유</span>
                  )}
                  {li.rm === "정" && (
                    <span style={{ backgroundColor: "#E8F1EE" }}>정</span>
                  )}
                  {li.rm === "유정" && (
                    <span style={{ backgroundColor: "#F7F7FB" }}>유정</span>
                  )}
                  {li.rm === "공" && (
                    <span style={{ backgroundColor: "#CDDDEA" }}>공</span>
                  )}
                  {li.rm === "연" && (
                    <span style={{ backgroundColor: "#FFF2DD" }}>연</span>
                  )}
                </td>
              </tr>
            );
